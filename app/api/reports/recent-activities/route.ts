...
/** GET /api/reports/recent-activities?store_id&user_id&limit  (owner only) */
export async function GET(req: Request) {
  ...
  if (!isSupabaseConfigured()) {
    return NextResponse.json({ ok: true, activities: mockActivities.slice(0, limit) });
  }
  ...
  const { data, error } = await supabase.rpc("rpc_get_recent_activities", {
    p_store_id: storeId,
    p_limit_count: limit,
  });
