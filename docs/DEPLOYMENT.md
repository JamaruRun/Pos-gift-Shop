...
- `vercel.json` (มีในรูट) ตั้ง cron ยิง `/api/notifications/process`:
  ```json
  { "crons": [{ "path": "/api/notifications/process", "schedule": "0 0 * * *" }] }
  ```
...
> ⚠️ **Vercel Hobby plan รัน cron ได้แค่วันละครั้ง** (`0 0 * * *` = เที่ยงคืน UTC) — ไม่รองรับ `* * * * *`
> ผลกระทบ: LINE แจ้งเตือนจะถูกส่งเป็น batch วันละครั้ง ไม่ใช่ near-real-time
>
> **สำหรับทดสอบ demo LINE:** เรียก worker เองด้วยมือ (ดูคำสั่งด้านล่าง)
> **สำหรับ near-real-time จริง:** อัปเกรด **Vercel Pro** (cron ถี่กว่าได้) หรือใช้ scheduler ภายนอก
> (เช่น Supabase pg_cron / cron-job.org / GitHub Actions schedule) ยิง POST มาที่ route นี้ถี่ขึ้น

#### เรียก worker ด้วยมือ (demo / ทดสอบ)
```bash
curl -X POST https://<your-app>.vercel.app/api/notifications/process \
  -H "Authorization: Bearer ${CRON_SECRET}"
# คืน { ok, processed, sent, failed } — ส่งทุก notification ที่ค้างใน queue ทันที
```
...
LINE end-to-end (Hobby): เปิดกะ → ขาย 1 บิล → **เรียก worker ด้วยมือ** (curl ข้างบน) → เจ้าของได้ LINE 🛒
(บน Pro/scheduler ภายนอกจะส่งอัตโนมัติตามรอบที่ตั้ง)
...
| **Vercel Hobby cron วันละครั้งเท่านั้น** (`0 0 * * *`) | 🟡 | LINE ส่ง batch รายวัน; demo เรียก worker ด้วยมือ; near-real-time = Vercel Pro / scheduler ภายนอก |
